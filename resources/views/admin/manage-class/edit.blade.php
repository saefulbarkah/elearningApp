    ...
    <div class="row">
        <div class="col">
            <div class="card">
                ...
                <div class="card-body">
                    <form method="POST" action="{{ url('admin/manage-class/'.$data->id.'/update') }}">
                        @csrf
                        <!-- Kelas -->
                        <div class="form-group">
                            ...
                            <select name="grade_id" class="form-control custom-select @error('grade_id') is-invalid @enderror"
                                ...
                                @foreach ($grade as $gd)
                                    <option value="{{ $gd->id }}" {{ ($data->grade_id == $gd->id) ? 'selected' : '' }}>{{ $gd->name }}</option>
                                @endforeach
                            </select>
                            @error('grade_id')
                        ...
                        <div class="form-group">
                            ...
                            <select name="major_id"
                                ...
                                @foreach ($major as $mj)
                                    <option value="{{ $mj->id }}" {{ ($data->major_id == $mj->id) ? 'selected' : '' }}>{{ $mj->name }}</option>
                                @endforeach
                            </select>
                            @error('major_id')
                                <div class="text-danger">* {{ $message }}</div>
