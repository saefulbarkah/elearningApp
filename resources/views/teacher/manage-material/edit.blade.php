...
<div class="row">
    <div class="col">
        <div class="card">
            ...
            <div class="card-body">
                <form action="{{ url('teacher/manage-material/update',$material->id) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    <div class="form-group">
                        <label for="exampleInputEmail1">Judul materi</label>
                        <input type="text" value="{{ $material->title }}" class="form-control @error('title') is-invalid @enderror" name="title"
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" disabled value="{{ $findMateri->name }}" class="form-control"
                            id="exampleInputEmail1" aria-describedby="emailHelp" autocomplete="off">
                        <input type="hidden" value="{{ $findMateri->id }}" name="subject_id" class="form-control"
                            id="exampleInputEmail1" aria-describedby="emailHelp" autocomplete="off">
                    </div>
                    <div class="form-group">
                        ...
                        is-invalid
                         @enderror" name="grade_major_id">
                            ...
                            <option value="{{ $item->gm_id }}"{{ ($item->gm_id == $material->grade_major_id) ? 'selected' : '' }}>{{ $item->grade_name }} | {{ $item->major_name }} {{ $item->group }}</option>
                            @endforeach
                         </select>
                    ...
                    <div class="form-group">
                        ...
                        <textarea class="form-control @error('description')
                            is-invalid
                        @enderror" name="description" id="exampleFormControlTextarea1" rows="3">{{ $material->description }}</textarea>
                        @error('description')
                        <div class="text-danger">* {{ $message }}</div>
                        @enderror
                    </div>
                    <div class="form-group mb-4">
                        ...
                        <div class="custom-file mb-3">
                            <input type="file" value="{{ $material->file }}" class="custom-file-input" name="file">
