    ...
    <div class="row">
        <div class="col">
            <div class="card">
                ...
                <div class="card-body">
                    <form action="{{ url('teacher/manage-material/update', $material->id) }}" method="POST"
                        ...
                        <div class="form-group">
                            ...
                            <select
                                class="js-example-basic-single form-control @error('grade_major_id') is-invalid @enderror"
                                name="grade_major_id">
                                <option selected="" disabled="">---Pilih Kelas dan jurusan---</option>
                                @foreach ($gradeMajors as $item)
                                    <option value="{{ $item->gm_id }}"
                                        {{ $item->gm_id == $material->grade_major_id ? 'selected' : '' }}>
                                        {{ $item->grade_name }} | {{ $item->major_name }} {{ $item->group }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <textarea class="form-control @error('description') is-invalid @enderror" name="description"
                                id="exampleFormControlTextarea1" rows="3">{{ $material->description }}</textarea>
                            @error('description')
                                <div class="text-danger">* {{ $message }}</div>
                            @enderror
                        </div>
